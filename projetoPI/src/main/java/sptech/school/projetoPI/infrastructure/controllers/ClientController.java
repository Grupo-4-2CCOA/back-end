...
import sptech.school.projetoPI.core.application.dto.user.client.ClientRequestDto;
import sptech.school.projetoPI.core.application.dto.user.client.ClientResponseDto;
...
public class ClientController {
    ...
    public ResponseEntity<UserResumeResponseDto> createClient(@Valid @RequestBody UserRequestDto requestDto) {
        UserDomain clientDomain = UserMapper.toDomain(requestDto);
        UserDomain createdClientDomain = createClientUseCase.execute(clientDomain);
        return new ResponseEntity<>(UserMapper.toResumeResponseDto(createdClientDomain), HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<UserResponseDto> getClientById(@PathVariable Integer id) {
        UserDomain clientDomain = getClientByIdUseCase.execute(id);
        return ResponseEntity.ok(UserMapper.toResponseDto(clientDomain));
    }
    ...
    public ResponseEntity<UserResumeResponseDto> updateClientById(@Valid @RequestBody UserRequestDto requestDto, @PathVariable Integer id) {
        UserDomain clientDomain = UserMapper.toDomain(requestDto);
        UserDomain updatedClientDomain = updateClientByIdUseCase.execute(clientDomain, id);
        return ResponseEntity.ok(UserMapper.toResumeResponseDto(updatedClientDomain));
    }
